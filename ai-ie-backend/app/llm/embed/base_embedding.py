...
# from app.db.models import APIType
# from app.db.ops import db_ops
from app.llm.embed.embedding_service import EmbeddingService
...
def _get_embedding_model(
    ...
    try:
        ...
        if dimensions is not None:
            embedding_dim = min(embedding_dim, dimensions)
...
def get_collection_embedding_service_sync(collection) -> tuple[EmbeddingService, int]:
    ...
    embedding_service_api_key = settings.embedding_service_api_key
    ...
    try:
        # 有优化空间，可以在embedding_service_api_key查询中同时带出来
        # llm_provider = db_ops.query_llm_provider_by_name(embedding_msp)
        llm_provider = config.embedding.custom_llm_provider
    ...
    try:
        multimodal = False
        # 后续可以考虑在此表中存储embedding模型的 dimension，目前存在歧义，一个模型可能有不同的 dimension
        # model = db_ops.query_llm_provider_model(embedding_msp, APIType.EMBEDDING.value, embedding_model_name)
        model = json.loads(settings.embedding_model_tags)
