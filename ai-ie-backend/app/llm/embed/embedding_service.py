...
from app.llm.llm_error_types import (
...
class EmbeddingService:
    ...
    def embed_documents(self, contents: List[str], dimensions: Optional[int] = None) -> List[List[float]]:
        """
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as pool:
                ...
                for start in range(0, len(clean_contents), batch_size):
                    ...
                    # future 实例
                    future = pool.submit(self._embed_batch_with_indices, batch, start, dimensions)
    ...
    def _embed_batch_with_indices(self, batch: Sequence[str], start_idx: int, dimensions: Optional[int] = None) -> List[Tuple[int, List[float]]]:
        """Process a batch of texts and return embeddings with their original indices."""
        try:
            embeddings = self._embed_batch(batch, dimensions)
    ...
    def _embed_batch(self, batch: Sequence[str], dimensions: Optional[int] = None) -> List[List[float]]:
        """
        ...
        try:
            # litellm bug, inadequate functional support
