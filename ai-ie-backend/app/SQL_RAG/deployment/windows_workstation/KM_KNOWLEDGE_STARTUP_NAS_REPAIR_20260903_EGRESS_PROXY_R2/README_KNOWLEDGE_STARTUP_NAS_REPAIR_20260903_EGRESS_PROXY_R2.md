...
版本：`KM_KNOWLEDGE_STARTUP_NAS_REPAIR_20260903_EGRESS_PROXY_R2`
...
R2 保留 R1 的全部出口门禁和代理覆盖，仅修正启动器在 Windows PowerShell 5.1
中的返回值兼容性：`Get-KmEgressProxyContract` 返回的泛型 `List[object]` 现在通过
`@($candidates|ForEach-Object{$_})` 显式物化，避免 `PSToObjectArrayBinder`
触发 `Argument types do not match`。R2 不重建 1534 基线，不改解析、提取、提示词、
数据库、队列、Worker 业务逻辑或第一套运行时。
...
```powershell
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "\\win2024-server\项目升级文件\王映钦\王映钦_ERP\SQL_RAG_packages\win11_workstation\KM_KNOWLEDGE_STARTUP_NAS_REPAIR_20260903_EGRESS_PROXY_R2\Install-MonFangAiKnowledgeStartupNasRepair.ps1" -SuiteRoot "D:\MonFangAI\GetDAM" -ValidateOnly
```
...
```powershell
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "\\win2024-server\项目升级文件\王映钦\王映钦_ERP\SQL_RAG_packages\win11_workstation\KM_KNOWLEDGE_STARTUP_NAS_REPAIR_20260903_EGRESS_PROXY_R2\Install-MonFangAiKnowledgeStartupNasRepair.ps1" -SuiteRoot "D:\MonFangAI\GetDAM" -StartServices
```
