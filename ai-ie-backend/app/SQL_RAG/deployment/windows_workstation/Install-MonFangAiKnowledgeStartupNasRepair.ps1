...
# [2026-09-03 12:02:00] 作用：读取包内第二套出口代理合同；理由依据：安装器必须在写入目标前确认冷启动所需的外部 LLM 出口事实已经随包发布。
$payloadEgressProxyProperty=$payloadServerProfile.docker.PSObject.Properties['egress_proxy']
# [2026-09-03 12:02:00] 作用：阻断缺少出口代理合同的载荷；理由依据：没有代理候选和探针地址会把已知 58% 超时重新交给业务 Worker。
if($null-eq$payloadEgressProxyProperty-or$null-eq$payloadEgressProxyProperty.Value){throw '第二套出口代理合同缺失。'}
# [2026-09-03 12:02:00] 作用：读取出口合同结构化对象；理由依据：候选、Worker 和探针必须从同一 profile 属性验证而不能依赖文本猜测。
$payloadEgressProxy=$payloadEgressProxyProperty.Value
# [2026-09-03 12:02:00] 作用：验证供应商探针地址；理由依据：只读 HTTPS 401/403 能证明目标容器确实建立了公网 TLS 出口且不携带密钥。
if([string]$payloadEgressProxy.probe_url-ne'https://api.siliconflow.cn/v1/models'){throw '第二套出口代理探针地址不符合合同。'}
# [2026-09-03 12:02:00] 作用：读取包内代理候选集合；理由依据：Docker Desktop 网关可能随重启变化，必须保留已验证的有序候选。
$payloadEgressCandidates=@($payloadEgressProxy.candidates)
# [2026-09-03 12:02:00] 作用：阻断空候选或缺少首个已验证网关；理由依据：当前唯一已实测可达地址是 192.168.65.7:3128，不能让启动器无代理启动。
if($payloadEgressCandidates.Count-lt1-or[string]$payloadEgressCandidates[0].url-ne'http://192.168.65.7:3128'){throw '第二套出口代理候选不包含当前已验证网关。'}
# [2026-09-03 12:02:00] 作用：读取出口 Worker 集合；理由依据：代理只允许注入三类外部模型 Worker，其他队列和持久化服务必须保持原环境。
$payloadEgressWorkers=@($payloadEgressProxy.worker_services|ForEach-Object{[string]$_}|Sort-Object)
# [2026-09-03 12:02:00] 作用：验证出口 Worker 集合精确匹配；理由依据：范围漂移会把代理扩散到 RabbitMQ、Redis、MinIO 或数据库。
if(($payloadEgressWorkers-join',')-ne'km-worker-asr,km-worker-llm,km-worker-vision'){throw '第二套出口代理 Worker 集合不符合合同。'}
# [2026-09-03 14:18:00] 作用：确认启动器包含显式 stdin、interactive 容器探针、实际商业网络复核和持久 override；理由依据：静态包门禁必须阻断空 stdin、只测 default bridge 或只写环境变量的半修复。
if(-not$commercialText.Contains('Invoke-KmDockerPythonScript')-or-not$commercialText.Contains("'--interactive'")-or-not$commercialText.Contains("'--network',`$Network")-or-not$commercialText.Contains("-Network 'bridge'")-or-not$commercialText.Contains('-Network $commercialNetwork')-or-not$commercialText.Contains('docker-compose.egress.override.yml')-or-not$commercialText.Contains('KNOWLEDGE_COMMERCIAL_EGRESS_PROXY_READY')-or-not$commercialText.Contains('KNOWLEDGE_COMMERCIAL_EGRESS_PROXY_NETWORK_READY')){throw '第二套出口代理 stdin、容器网络探针或 override 合同不成立。'}
